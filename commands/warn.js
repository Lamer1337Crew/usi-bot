...
exports.run = async (client, message, args) => {
  ...
  if (message.mentions.users.first().id === "482706815784714272") return message.reply("You can't warn my Developer:wink:");
  ...
  if(warns[`${user.id}, ${message.guild.id}`].warns == 6){
    message.guild.member(user).kick(reason);
    message.reply('That Dumb Boi have been kicked :facepalm:')
  }

  if(warns[`${user.id}, ${message.guild.id}`].warns == 9){
